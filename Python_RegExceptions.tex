...
print \verb|"<h2> Selected Subject is  $  \%  $s  $  \%  $s</h2>"  $  \%  $ subject

print \"</body>\"

print \"</html>\"

<form action=\verb|"/cgi-bin/textarea.py" method="post" target=" $  \_  $blank">
...
print \verb|"<h2> Entered Text Content is  $  \%  $s</h2>"  $  \%  $ text $  \_  $content
...
~ Match = re.search ('([ $  \setminus  $ w .-] +) @ ([ $  \setminus  $ w .-] +)', str)
...
~ Email = re.findall (r '[ $  \setminus  $ w  $  \setminus  $ .-] + @ [ $  \setminus  $ w  $  \setminus  $ .-] +', str)  $  \#  $ $  \#  $ ['alice@google.com', 'bob@abc.com']
